...
class AgentClient:
    ...
    def __init__(self):
        ...
        self.agent_endpoint = self._normalize_project_endpoint(os.getenv("AGENT_ENDPOINT"))
        if not self.agent_endpoint:
            raise ValueError("AGENT_ENDPOINT not found in environment variables")

        self.agent_name = os.getenv("AGENT_NAME", "computing-historian")
        self.agent_version = os.getenv("AGENT_VERSION", "1")
    ...
    @staticmethod
    def _normalize_project_endpoint(endpoint: str | None) -> str:
        """Return the Foundry project endpoint expected by AIProjectClient."""
        if not endpoint:
            return ""
    ...
    def send_message(self, user_message: str) -> str:
        ...
        try:
            ...
            raise RuntimeError("Unable to communicate with the agent. Check Azure authentication and endpoint settings.") from e
